    ...
    def get(self, k):
        ...
        Args:
            k: key of the named data; this could also include reference to a sub-property of the named object
               ...
               2. `a.b.c` where `b` is a dict - it would try to retrieve data named `a` (main object),
                  then its property `b`, and then finally return `c` as the key of `b`
               3. `a.b.0` where `b` is a list/tuple - it would try to retrieve data named `a` (main object),
                  then its property `b`, and then finally return its first value
        ...
        Raises:
            ...
            IndexError: if the main object does exist in the data, but the object (or its property) is
                        a list/tuple that does not have any value at the given index
            AttributeError: if the main object does exist in the data, but the object (or its property):
                            ...
                            b) is a dict and does not have the requested key
        """
        ...
        for index, part in enumerate(parts[1:]):
            ...
            try:
                if isinstance(value, dict):
                    value = value[part]
                elif isinstance(value, (list, tuple)) and part.isnumeric():
